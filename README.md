# quay-registry
Use [RedHat mirror-registry package](https://docs.openshift.com/container-platform/4.14/installing/disconnected_install/installing-mirroring-creating-registry.html) to create a streamlined Quay container registry for your lab environment.  <br>
...
SHORT:   podman ps -a --pod <br>
LONG:    podman ps -a --pod --format json  
...
The first systemd service is to setup the pod (podman pod create), and the next three (3) systemd services will add containers to the pod (--pod=quay-pod), to share networking and other shared services.  
...
### Testing with skopeo  
The [skopeo](https://github.com/containers/skopeo) package allows us to bypass the standard docker/podman 'pull/tag/push' process to a single 'push' process.
```
...
```
We can now see the new container loaded (with the SAME exact SHA digest manifest)
